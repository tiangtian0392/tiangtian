...
def select_file_dialog():
    ...
    if not file_path or not file_path.endswith('.xlsx'):
        messagebox.showerror('错误', '没有选择文件或文件不是xlsx文件，程序退出！')
        sys.exit()
    print(file_path)
    return file_path
...
class OrderDataProcessor:
    ...
    def generate_output_files(self, Qoo10data_file, zaiku_file_path, output_folder):
        # 读取第一个表和在库表的数据
        Qdata = pd.read_csv(Qoo10data_file, encoding='shift-jis')
        zaiku_data = pd.read_csv(zaiku_file_path, encoding='shift-jis')
...
# 获取文件路径
Qoo10data_file = processor.select_file_dialog()
print(Qoo10data_file)
#在库文件路径
zaiku_file_path = r"\\LS410D8E6\tool\bazhuayu\在庫.csv"
# 生成输出文件
processor.generate_output_files(Qoo10data_file, zaiku_file_path, r"D:\\")