...
class ExcelHandler:
    ...
    def _convert_cell_name_to_indices(self, cell_name):
        ...
        if match:
            ...
            # print(f'返回转换后的行列索引：row= {int(row)},clo={col}')
            return int(row), col
        else:
    ...
    def insert_row(self, sheet_name, row):
        """
        在指定行位置插入新行
        :param sheet_name: 表名
        :param row: 用于在指定行之前插入新行。整数，如2
        :return:
        """
        sheet = self.workbook.Sheets(sheet_name)
        sheet.Rows(row).Insert()

    def insert_column(self, sheet_name, col):
        """
        在指定列位置插入新列
        col 为列名，如 'K'
        """
        # sheet = self.workbook.Sheets(sheet_name)
        # col_index = self._convert_cell_name_to_indices(col)[1]
        # print(f'插入列：col={col},col_index={col_index}')
        # sheet.Columns(col_index).Insert(Shift=win32com.client.constants.xlToRight)

        sheet = self.workbook.Sheets(sheet_name)
        col_index = self._convert_cell_name_to_indices(col)[1]
        print(f'插入列：col={col},col_index={col_index}')
        sheet.Columns(col_index).Insert()
